...
import { AUTH_ERROR, LOGOUT, SIGNIN, SIGNIN_REQUEST,SIGNUP,SIGNUP_REQUEST, UPADTE_PROFILE, UPDATE_PROFILE_ERROR, UPDATE_PROFILE_REQ , UPDATE_SUBSCRIPTION_REQ, UPADTE_SUBSCRIPTION , UPDATE_SUBSCRIPTION_ERROR} from "./types"
...
export const updateSubscription=(userInfo)=>async (dispatch,getState)=>{
    dispatch({type:UPDATE_SUBSCRIPTION_REQ,payload:userInfo})
    try{
        const user = getState().user?.user;
      const {data} = await pizza.put('/api/users/updateSubscription',userInfo,{
        headers:{
            Authorization: `Bearer ${user.token}`
         }
      })
      dispatch({type:UPADTE_SUBSCRIPTION,payload:data})
      localStorage.setItem('User',JSON.stringify(data))
     console.log(data)
    }
    catch(error){
        dispatch({type:UPDATE_SUBSCRIPTION_ERROR,payload:error.response&&error.response.data.message?error.response.data.message:error.message})
    }
}
